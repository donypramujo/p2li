...
<ul class="breadcrumb no-border no-radius b-b b-light pull-in">
	...
	<li class="active">{{trans('app.breeder.edit')}}</li>
</ul>

<div class="m-b-md">
	<h3 class="m-b-none">{{trans('app.breeder.edit')}}</h3>
</div>
<section class="panel panel-default">
	<header class="panel-heading font-bold">{{trans('app.breeder.breeder')}}</header>
	<div class="panel-body">
		<form action="{{action('BreederController@update',$breeder->id)}}" method="post" class="form-horizontal" autocomplete="off">
			{!! csrf_field() !!}
			{{ method_field('PUT') }}
			<div class="form-group {{$errors->has('name') ? 'has-error' : '' }}">
				<label class="col-sm-2 control-label font-bold">{{trans('app.breeder.name')}}</label>
				<div class="col-sm-10">
					<input type="text" name="name" class="form-control" value="{{ $errors->has() ? old('name') : $breeder->name }}" maxlength="50">
